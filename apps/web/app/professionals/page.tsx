...
function ProfessionalsContent() {
  ...
  useEffect(() => {
    setLoading(true);
    getProfessionalProfiles(selected || undefined)
      .then(setProfiles)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Browse degree-holding professionals for full-time & contract roles</p>
      </div>
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="card animate-pulse h-44 bg-slate-100" />)}
        </div>
      ) : profiles.length === 0 ? (
        <div className="text-center py-20 text-slate-400">
          <div className="text-5xl mb-4">💼</div>
          <p className="text-lg font-medium">No professionals found yet.</p>
