...
import { fetchWorkableJobs } from '@/lib/job-ingestion/workable';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const jobs = await fetchWorkableJobs(source);
    await upsertImportedJobs(jobs, { provider: 'workable', sourceKey: source.sourceKey, companyName: source.companyName, careersUrl: source.careersUrl });
    return NextResponse.json({ provider: 'workable', sourceKey: source.sourceKey, imported: jobs.length });
  } catch (error: any) {
    console.error('[workable ingestion]', error);
    return NextResponse.json({ error: error.message ?? 'Workable ingestion failed.' }, { status: 500 });
  }
}
